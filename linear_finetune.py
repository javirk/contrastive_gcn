#
# Original work by: Wouter Van Gansbeke & Simon Vandenhende
...
from libs.common_config import get_image_transforms, get_train_transformations, \
    get_dataset, get_val_transformations, \
...
def main():
    ...
    # CUDNN
    print(('Set CuDNN benchmark', 'blue'))
    torch.backends.cudnn.benchmark = True

    # Optimizer
    print(('Retrieve optimizer', 'blue'))
    ...
    print(('Train samples %d - Val samples %d' % (len(train_dataset), len(val_dataset)), 'yellow'))
    ...
    if os.path.exists(p['checkpoint']):
        ...
        print(('No checkpoint file at {}'.format(p['checkpoint']), 'blue'))
        start_epoch = 0
        best_epoch = 0
        best_iou = 0
        # model = model.cuda()
    ...
    for epoch in range(start_epoch, p['epochs']):
        print(('Epoch %d/%d' % (epoch + 1, p['epochs']), 'yellow'))
        print(('-' * 10, 'yellow'))
    ...
    print(('Evaluating best model at the end', 'blue'))
